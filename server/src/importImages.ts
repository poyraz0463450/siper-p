...
async function extractImagesFromExcel(excelPath: string, outputDir: string = 'extracted_images') {
...
async function updatePartsWithImages(imageMatches: ImageMatch[], imageDir: string) {
...
async function main() {
    ...
    if (args.length < 1) {
        console.log('Usage: npx ts-node src/importImages.ts <path-to-excel-file> [output-directory]');
        console.log('Example: npx ts-node src/importImages.ts "D:\\Downloads\\Kitap2.xlsx"');
        process.exit(1);
    }
